  ...
  useEffect(() => {
    const postData = async () => {
      if (step === 3) {
        const data = {
          ...formData,
          canvasUrl: 'https://' + formData?.canvasUrl + '/api/v1/',
          canvasToken: formData?.canvasKey,
        }
        // console.log(data)
  ...
  return (
    <div className="bg-card border-2 border-border flex flex-col w-[500px] px-5 py-3 h-[450px] rounded-xl gap-3">
      <div className="border-b-2 border-border">
        <h1 className="text-2xl font-bold py-3">Onboarding</h1>
      </div>
      <div className="flex flex-col h-full gap-10">
        ...
        {step === 3 && (
          <motion.div
            ...
            className="flex flex-col h-[60%] items-center w-full justify-center gap-2"
          >
